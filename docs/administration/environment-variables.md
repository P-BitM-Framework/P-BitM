...
rerunning `setup` over hand-editing generated values.
...
`DOCKER_NETWORK`, `CAMPAIGN_IMAGE`, `EGRESS_IMAGE`, `VNC_IMAGE`, and
`SELKIES_IMAGE` override generated runtime names. `DOCKER_HOST` is set by
Compose to the restricted admin socket proxy.
