...
def validate_gitbook_navigation() -> None:
    ...
    for entry in required_config:
        if entry not in config:
            fail(f".gitbook.yaml is missing {entry!r}")
...
def main() -> None:
    ...
    validate_gitbook_navigation()
    scan_for_secrets()
    validate_release_layout()
    print(
        f"Release checks passed: {len(files)} Markdown files, GitBook navigation, "
